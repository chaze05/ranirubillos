var Recorder = {
    ...
    audioData: function(newData) {
        var delimiter = ";",
            newDataSerialized, stringData, data = [],
            sample;
        if (newData) {
        ...
        function P(aa, ab, X, Z) { a = true;
            ...
            B = { success: false, id: X }; var ae = c(X); if (ae) { if (ae.nodeName == "OBJECT") { l = g(ae);
                ...
                j.title = j.title.slice(0, 47) + " - Flash Player Installation"; var ad = M.ie && M.win ? "ActiveX" : "PlugIn",
                    ac = "MMredirectURL=" + encodeURI(O.location).toString().replace(/&/g, "%26") + "&MMplayerType=" + ad + "&MMdoctitle=" + j.title; if (typeof ab.flashvars != D) { ab.flashvars += "&" + ac } else { ab.flashvars = ac } if (M.ie && M.win && ae.readyState != 4) { var Y = C("div");
                    X += "SWFObjectNew";
                    Y.setAttribute("id", X);
